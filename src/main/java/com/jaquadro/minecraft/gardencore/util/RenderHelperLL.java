...
public class RenderHelperLL {
   ...
   private static final int[][][] xyzuvMap = new int[][][]{{{0, 2, 5, 0, 3}, {0, 2, 4, 0, 2}, {1, 2, 4, 1, 2}, {1, 2, 5, 1, 3}}, {{1, 3, 5, 1, 3}, {1, 3, 4, 1, 2}, {0, 3, 4, 0, 2}, {0, 3, 5, 0, 3}}, {{0, 3, 4, 1, 2}, {1, 3, 4, 0, 2}, {1, 2, 4, 0, 3}, {0, 2, 4, 1, 3}}, {{0, 3, 5, 0, 2}, {0, 2, 5, 0, 3}, {1, 2, 5, 1, 3}, {1, 3, 5, 1, 2}}, {{0, 3, 5, 1, 2}, {0, 3, 4, 0, 2}, {0, 2, 4, 0, 3}, {0, 2, 5, 1, 3}}, {{1, 2, 5, 0, 3}, {1, 2, 4, 1, 3}, {1, 3, 4, 1, 2}, {1, 3, 5, 0, 2}}};
   ...
   private void drawFaceY(int face, double x, double y, double z, IIcon icon) {
      ...
      if (rangeX <= 1 && rangeZ <= 1) {
         this.setUV(icon, this.state.renderMinX + (double)this.state.shiftU, this.state.renderMinZ + (double)this.state.shiftV, this.state.renderMaxX + (double)this.state.shiftU, this.state.renderMaxZ + (double)this.state.shiftV);
   ...
   private void drawFaceZ(int face, double x, double y, double z, IIcon icon) {
      ...
      if (rangeX <= 1 && rangeY <= 1) {
         if (this.state.flipTexture) {
            this.setUV(icon, this.state.renderMaxX + (double)this.state.shiftU, 1.0D - this.state.renderMaxY + (double)this.state.shiftV, this.state.renderMinX + (double)this.state.shiftU, 1.0D - this.state.renderMinY + (double)this.state.shiftV);
         } else {
            this.setUV(icon, this.state.renderMinX + (double)this.state.shiftU, 1.0D - this.state.renderMaxY + (double)this.state.shiftV, this.state.renderMaxX + (double)this.state.shiftU, 1.0D - this.state.renderMinY + (double)this.state.shiftV);
         }
         ...
         for(int ix = 0; ix < rangeX; ++ix) {
            ...
            for(int iy = 0; iy < rangeY; ++iy) {
               ...
               if (this.state.flipTexture) {
                  this.setUV(icon, 1.0D - this.minUDiv[ix], 1.0D - this.maxVDiv[iy], 1.0D - this.maxUDiv[ix], 1.0D - this.minVDiv[iy]);
               } else {
                  this.setUV(icon, this.minUDiv[ix], 1.0D - this.maxVDiv[iy], this.maxUDiv[ix], 1.0D - this.minVDiv[iy]);
               }
   ...
   private void drawFaceX(int face, double x, double y, double z, IIcon icon) {
      ...
      if (rangeZ <= 1 && rangeY <= 1) {
         if (this.state.flipTexture) {
            this.setUV(icon, this.state.renderMaxZ + (double)this.state.shiftU, 1.0D - this.state.renderMaxY + (double)this.state.shiftV, this.state.renderMinZ + (double)this.state.shiftU, 1.0D - this.state.renderMinY + (double)this.state.shiftV);
         } else {
            this.setUV(icon, this.state.renderMinZ + (double)this.state.shiftU, 1.0D - this.state.renderMaxY + (double)this.state.shiftV, this.state.renderMaxZ + (double)this.state.shiftU, 1.0D - this.state.renderMinY + (double)this.state.shiftV);
         }
         ...
         for(int iz = 0; iz < rangeZ; ++iz) {
            ...
            for(int iy = 0; iy < rangeY; ++iy) {
               ...
               if (this.state.flipTexture) {
                  this.setUV(icon, 1.0D - this.minUDiv[iz], 1.0D - this.maxVDiv[iy], 1.0D - this.maxUDiv[iz], 1.0D - this.minVDiv[iy]);
               } else {
                  this.setUV(icon, this.minUDiv[iz], 1.0D - this.maxVDiv[iy], this.maxUDiv[iz], 1.0D - this.minVDiv[iy]);
               }
