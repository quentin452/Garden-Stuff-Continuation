...
public class RenderHelper {
   ...
   public void setRenderBounds(Block block) {
      this.setRenderBounds(block.getBlockBoundsMinX(), block.getBlockBoundsMinY(), block.getBlockBoundsMinZ(), block.getBlockBoundsMaxX(), block.getBlockBoundsMaxY(), block.getBlockBoundsMaxZ());
   }
   ...
   public void renderCrossedSquares(IBlockAccess blockAccess, Block block, int x, int y, int z) {
      this.renderCrossedSquares(blockAccess, block, x, y, z, this.getBlockIconFromSideAndMetadata(block, 0, blockAccess.getBlockMetadata(x, y, z)));
   }
   ...
   public void drawCrossedSquaresBounded(IIcon icon, double x, double y, double z, float scale) {
      ...
      tessellator.addVertexWithUV(xNN, yMin, zNN, uNN, vMax);
      tessellator.addVertexWithUV(xNN, yMax, zNN, uNN, vMin);
      double uNP = (double)icon.getInterpolatedU(Math.max(this.state.renderMinX, 1.0D - this.state.renderMaxZ) * 16.0D);
      double uPN = (double)icon.getInterpolatedU(Math.min(this.state.renderMaxX, 1.0D - this.state.renderMinZ) * 16.0D);
      ...
      uNP = (double)icon.getInterpolatedU(16.0D - Math.max(this.state.renderMinX, 1.0D - this.state.renderMaxZ) * 16.0D);
      uPN = (double)icon.getInterpolatedU(16.0D - Math.min(this.state.renderMaxX, 1.0D - this.state.renderMinZ) * 16.0D);
      tessellator.addVertexWithUV(xPN, yMax, zPN, uPN, vMin);
      tessellator.addVertexWithUV(xPN, yMin, zPN, uPN, vMax);
      tessellator.addVertexWithUV(xNP, yMin, zNP, uNP, vMax);
      tessellator.addVertexWithUV(xNP, yMax, zNP, uNP, vMin);
   ...
   private IIcon getIconSafe(IIcon icon) {
      return (IIcon)(icon == null ? ((TextureMap)Minecraft.getMinecraft().getTextureManager().getTexture(TextureMap.locationBlocksTexture)).getAtlasSprite("missingno") : icon);
   }
}
