    ...
    public ContainerGarden(InventoryPlayer inventory, TileEntityGarden tileEntity) {
        ...
        for (i = 0; i < storageSlotIndex.length; ++i) {
            if (tileEntity.isSlotValid(storageSlotIndex[i])) {
                this.storageSlots.add(
                    this.addSlotToContainer(
                        new PlantSlot(
                            this.gardenInventory,
                            storageSlotIndex[i],
                            44 + 18 * storageXIndex[i],
                            18 + 18 * storageYIndex[i])));
            }
        }
        ...
        for (i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.playerSlots
                    .add(this.addSlotToContainer(new Slot(inventory, j + i * 9 + 9, 8 + j * 18, 122 + i * 18)));
            }
        }
