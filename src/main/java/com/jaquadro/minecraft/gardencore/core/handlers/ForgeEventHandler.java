...
public class ForgeEventHandler {
    ...
    public void playerInteracts(PlayerInteractEvent event) {
        if (event.action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (stack != null && stack.getItem() == GameData.getItemRegistry()
                .getObject("Botania:flowerBag")) {
        ...
        if (event.action == Action.LEFT_CLICK_BLOCK) {
            if (event.entityPlayer.capabilities.isCreativeMode) {
                if (BlockSmallFire
                    .extinguishSmallFire(event.world, (EntityPlayer) null, event.x, event.y, event.z, event.face)) {
                    event.setResult(Result.ALLOW);
                    return;
                }
            } else {
                Block block = event.world.getBlock(event.x, event.y, event.z);
                if (!block.isAir(event.world, event.x, event.y, event.z) && BlockSmallFire
                    .extinguishSmallFire(event.world, (EntityPlayer) null, event.x, event.y, event.z, event.face)) {
                    event.setResult(Result.ALLOW);
                    return;
                }
