...
public abstract class BlockGardenContainer extends BlockGarden {
   ...
   protected boolean applyItemToGarden(World world, int x, int y, int z, EntityPlayer player, ItemStack itemStack, float hitX, float hitY, float hitZ, boolean hitValid) {
      ...
      if (item == null) {
         ...
         return this.applySubstrateToGarden(world, x, y, z, itemStack == null ? player : null, slot, item) ? true : super.applyItemToGarden(world, x, y, z, player, itemStack, hitX, hitY, hitZ, hitValid);
      }
   }
   ...
   protected boolean isValidSubstrate(World world, int x, int y, int z, int slot, ItemStack itemStack) {
      if (itemStack != null && itemStack.getItem() != null) {
         ...
         if (block == null) {
            ...
            return block == Blocks.dirt || block == Blocks.sand || block == Blocks.gravel || block == Blocks.soul_sand || block == Blocks.grass || block == Blocks.water || block == Blocks.farmland || block == Blocks.mycelium || block == ModBlocks.gardenSoil || block == ModBlocks.gardenFarmland;
         }
      } else {
         return false;
      }
   ...
   protected ItemStack translateSubstrate(World world, int x, int y, int z, int slot, ItemStack itemStack) {
      return Block.getBlockFromItem(itemStack.getItem()) == Blocks.farmland ? new ItemStack(Blocks.farmland, 1, 1) : itemStack;
   }
}
