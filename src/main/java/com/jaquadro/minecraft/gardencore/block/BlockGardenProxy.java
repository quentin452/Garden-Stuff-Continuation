...
public class BlockGardenProxy extends Block implements IPlantProxy {
    ...
    public float getPlantOffsetX(IBlockAccess blockAccess, int x, int y, int z, int slot) {
        ...
        return gardenBlock == null ? 0.0F
            : gardenBlock.getSlotProfile()
                .getPlantOffsetX(blockAccess, x, this.getBaseBlockYCoord(blockAccess, x, y, z), z, slot);
    }
    ...
    public float getPlantOffsetY(IBlockAccess blockAccess, int x, int y, int z, int slot) {
        ...
        return gardenBlock == null ? 0.0F
            : gardenBlock.getSlotProfile()
                .getPlantOffsetY(blockAccess, x, this.getBaseBlockYCoord(blockAccess, x, y, z), z, slot);
    }
    ...
    public float getPlantOffsetZ(IBlockAccess blockAccess, int x, int y, int z, int slot) {
        ...
        return gardenBlock == null ? 0.0F
            : gardenBlock.getSlotProfile()
                .getPlantOffsetZ(blockAccess, x, this.getBaseBlockYCoord(blockAccess, x, y, z), z, slot);
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (te == null) {
            ...
            if (garden == null) {
                ...
                int[] var9 = garden.getSlotProfile()
                    .getPlantSlots();
    ...
    public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB mask, List list,
        ...
        if (te != null && garden != null) {
            int baseY = this.getBaseBlockYCoord(world, x, y, z);
            int[] var11 = garden.getSlotProfile()
                .getPlantSlots();
                ...
                if (block != null) {
                    ...
                    try {
                        ...
                        Vec3 slotStartVec = Vec3.createVectorHelper(
                            startVec.xCoord - (double) offsetX,
                            startVec.yCoord - (double) offsetY,
                            startVec.zCoord - (double) offsetZ);
                        Vec3 slotEndVec = Vec3.createVectorHelper(
                            endVec.xCoord - (double) offsetX,
                            endVec.yCoord - (double) offsetY,
                            endVec.zCoord - (double) offsetZ);
                        MovingObjectPosition sub = block.collisionRayTrace(world, x, y, z, slotStartVec, slotEndVec);
                        if (mop == null
                            || slotStartVec.squareDistanceTo(mop.hitVec) > slotStartVec.squareDistanceTo(sub.hitVec)) {
                            mop = sub;
                        }
                    } catch (Exception var25) {} finally {
                        this.unbindSlot(world, x, y, z, te);
                    }
                }
            }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float vx, float vy,
        ...
        if (te != null && garden != null) {
            boolean flag = false;
            int[] var13 = garden.getSlotProfile()
                .getPlantSlots();
    ...
    public boolean applyBonemeal(World world, int x, int y, int z) {
        ...
        if (block == null) {
            ...
            int[] var8 = block.getSlotProfile()
                .getPlantSlots();
            int var9 = var8.length;

            for (int var10 = 0; var10 < var9; ++var10) {
                int slot = var8[var10];
                Iterator var12 = GardenCoreAPI.instance()
                    .getBonemealHandlers()
                    .iterator();
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        if (this.reeLightValue > 0) {
            ...
            BlockGarden garden = this.getGardenBlock(world, x, y, z);
            if (te != null && garden != null) {
                int[] var8 = garden.getSlotProfile()
                    .getPlantSlots();
                int var9 = var8.length;
    ...
    private int getBaseBlockYCoord(IBlockAccess world, int x, int y, int z) {
        if (y == 0) {
            ...
            for (Block underBlock = world.getBlock(x, y, z); y > 0
                && underBlock instanceof IPlantProxy; underBlock = world.getBlock(x, y, z)) {
                --y;
            }
