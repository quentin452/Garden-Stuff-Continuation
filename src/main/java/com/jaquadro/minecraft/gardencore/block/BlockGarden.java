...
public abstract class BlockGarden extends BlockContainer implements IGardenBlock {
   ...
   public void breakBlock(World world, int x, int y, int z, Block block, int data) {
      TileEntityGarden te = this.getTileEntity(world, x, y, z);
      if (te != null) {
         Iterator var8 = te.getReachableContents().iterator();
   ...
   protected boolean applyItemToGarden(World world, int x, int y, int z, EntityPlayer player, ItemStack itemStack, float hitX, float hitY, float hitZ, boolean hitValid) {
      ...
      if (item == null) {
         ...
         if (this.applyTestKitToGarden(world, x, y, z, slot, item)) {
            ...
            if (plant != null) {
               int plantSlot = hitValid ? this.getEmptySlotForPlant(world, x, y, z, player, plant, hitX, hitY, hitZ) : this.getEmptySlotForPlant(world, x, y, z, player, plant);
               if (plantSlot == -1) {
                  return false;
               } else {
                  return this.canSustainPlantIndependently(world, x, y, z, plant) ? false : this.applyPlantToGarden(world, x, y, z, itemStack == null ? player : null, plantSlot, plant);
               }
            } else {
               return false;
            }
   ...
   public boolean canSustainPlantIndependently(IBlockAccess blockAccess, int x, int y, int z, PlantItem plant) {
      ...
      return item instanceof IPlantable ? this.canSustainPlant(blockAccess, x, y, z, ForgeDirection.UP, (IPlantable)item) : false;
   }
         ...
         if (substrateStack != null && substrateStack.getItem() != null) {
            ...
            if (tileEntity == null) {
               ...
               for(int i = 0; i < 5; ++i) {
                  ...
                  world.spawnParticle("happyVillager", (double)((float)x + world.rand.nextFloat()), (double)((float)y + 0.5F + world.rand.nextFloat()), (double)((float)z + world.rand.nextFloat()), d0, d1, d2);
               }
