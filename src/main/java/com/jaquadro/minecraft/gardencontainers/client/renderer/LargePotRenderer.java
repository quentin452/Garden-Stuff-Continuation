...
public class LargePotRenderer implements ISimpleBlockRenderingHandler {
    ...
    private boolean renderWorldBlockPass0(IBlockAccess world, int x, int y, int z, BlockLargePot block, int modelId,
        RenderBlocks renderer) {
    ...
    private boolean renderWorldBlockPass1(IBlockAccess world, int x, int y, int z, BlockLargePot block, int modelId,
        RenderBlocks renderer) {
        ...
        if (tileEntity == null) {
            ...
            if (tileEntity.getSubstrate() != null && tileEntity.getSubstrate()
                .getItem() instanceof ItemBlock) {
                Block substrate = Block.getBlockFromItem(
                    tileEntity.getSubstrate()
                        .getItem());
                int substrateData = tileEntity.getSubstrate()
                    .getItemDamage();
                if (substrate == Blocks.water) {
                    IIcon substrateIcon = renderer.getBlockIconFromSideAndMetadata(substrate, 1, substrateData);
