...
public class ConfigManager {
    ...
    public ConfigManager(File file) {
        ...
        this.categoryPatterns.setComment(
        ...
        this.categoryPatternSettings.setComment(
        ...
        for (int i = 1; i < 256; ++i) {
            if (this.config.hasKey(this.categoryPatterns.getQualifiedName(), "pattern." + i)) {
                String entry = this.config.get(this.categoryPatterns.getQualifiedName(), "pattern." + i, "")
                    .getString();
    ...
    private void populateDefaultPattern() {
        ...
        String name = this.config.get(this.defaultPatternSettings.getQualifiedName(), "name", "Unknown")
            .getString();
        int weight = this.config.get(this.defaultPatternSettings.getQualifiedName(), "weight", 5)
            .getInt();
        this.defaultPattern.setName(name);
        this.defaultPattern.setWeight(weight);
        String gen = this.config.get("2_pattern_settings", "pattern_gen", "dungeonChest, 1; mineshaftCorridor, 1")
            .getString();
    ...
    private void parsePatternAttribs(PatternConfig pattern, String category) {
        ...
        if (this.config.hasKey(category, "weight")) {
            int weight = this.config.get(this.defaultPatternSettings.getQualifiedName(), "weight", 1)
                .getInt();
