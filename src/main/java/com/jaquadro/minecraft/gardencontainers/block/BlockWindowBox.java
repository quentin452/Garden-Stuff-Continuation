...
public class BlockWindowBox extends BlockGarden {
    ...
    public BlockWindowBox(String blockName, Material material) {
        ...
        this.slotProfile = new BlockWindowBox.LocalSlotProfile(
            new BasicSlotProfile.Slot[] {
                new BasicSlotProfile.Slot(0, new PlantType[] { PlantType.GROUND_COVER }, allSize),
                new BasicSlotProfile.Slot(1, commonType, smallSize),
                new BasicSlotProfile.Slot(2, commonType, smallSize),
                new BasicSlotProfile.Slot(3, commonType, smallSize),
                new BasicSlotProfile.Slot(4, commonType, smallSize) });
    }
    ...
    private void setBlockBounds(IBlockAccess world, int x, int y, int z, boolean validNW, boolean validNE,
        boolean validSW, boolean validSE) {
        TileEntityWindowBox te = this.getTileEntity(world, x, y, z);
        float yMin = te.isUpper() ? 0.5F : 0.0F;
        float yMax = te.isUpper() ? 1.0F : 0.5F;
    ...
    private class LocalSlotProfile extends Slot5Profile {
        ...
        private AxisAlignedBB[] mainClippingBounds = new AxisAlignedBB[] {
            AxisAlignedBB.getBoundingBox(0.3125D, 0.0D, 0.3125D, 0.6875D, 0.0625D, 0.6875D),
            AxisAlignedBB.getBoundingBox(0.25D, 0.0625D, 0.25D, 0.75D, 0.125D, 0.75D),
            AxisAlignedBB.getBoundingBox(0.1875D, 0.125D, 0.1875D, 0.8125D, 1.0D, 0.8125D) };
