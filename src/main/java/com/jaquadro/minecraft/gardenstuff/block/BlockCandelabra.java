...
public class BlockCandelabra extends BlockContainer {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (tile != null) {
            ...
            if (tile.isSconce()) {
                if (level == 0) {
                    RenderHelper.instance.state.transformCoord(
                        0.5D,
                        (double) flameDepth,
                        0.25D,
                        c,
                        RenderHelperState.ROTATION_BY_FACE_FACE[2][dir]);
                ...
                boolean hanging = level > 0 && (blockUpper instanceof IChain
                    || blockUpper.isSideSolid(world, x, y + 1, z, ForgeDirection.DOWN));
                if (level >= 0 && !hanging) {
                    RenderHelper.instance.state.transformCoord(
                        0.5D,
                        (double) (flameDepth + 0.0625F),
                        0.5D,
                        c,
                        RenderHelperState.ROTATION_BY_FACE_FACE[2][dir]);
                    this.renderParticleAt(world, (double) x + c[0], (double) y + c[1], (double) z + c[2]);
