...
public class BlockLightChain extends Block implements IPlantProxy, IChain {
    ...
    public Vec3[] getAttachPoints(IBlockAccess world, int x, int y, int z) {
        ...
        IAttachable attachable = GardenAPI.instance()
            .registries()
            .attachable()
            .getAttachable(bottomBlock, world.getBlockMetadata(x, y - 1, z));
        ...
        if (bottomBlock instanceof IChainAttachable) {
            ...
            attachPoints = new Vec3[] {
                Vec3.createVectorHelper(0.5D, attachable.getAttachDepth(world, x, y - 1, z, 1), 0.5D) };
