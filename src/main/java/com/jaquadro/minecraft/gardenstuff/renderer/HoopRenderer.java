...
public class HoopRenderer implements ISimpleBlockRenderingHandler {
    ...
    private void renderInventoryBlock(BlockHoop block, int metadata, int modelId, RenderBlocks renderer) {
        ...
        this.boxrender
            .renderBox((IBlockAccess) null, block, 0.0D, 0.0D, 0.0D, 0.0D, 0.0625D, 0.0D, 1.0D, 0.375D, 1.0D, 0, 3);
        GL11.glTranslatef(0.5F, 0.5F, 0.5F);
    }

    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        RenderBlocks renderer) {
        return !(block instanceof BlockHoop) ? false
            : this.renderWorldBlock(world, x, y, z, (BlockHoop) block, modelId, renderer);
    }

    private boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockHoop block, int modelId,
        RenderBlocks renderer) {
        this.boxrender.setUnit(0.0625D);
        this.boxrender.setIcon(block.getIcon(0, 0));
        this.boxrender.setColor(ModularBoxRenderer.COLOR_WHITE);
        this.boxrender
            .renderBox(world, block, (double) x, (double) y, (double) z, 0.0D, 0.0625D, 0.0D, 1.0D, 0.375D, 1.0D, 0, 3);
        return true;
    }
