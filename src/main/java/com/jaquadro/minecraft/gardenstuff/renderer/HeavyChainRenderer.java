...
public class HeavyChainRenderer implements ISimpleBlockRenderingHandler {
    ...
    private static final Vec3[] defaultAttachPoint = new Vec3[] { Vec3.createVectorHelper(0.5D, 0.0D, 0.5D),
        Vec3.createVectorHelper(0.5D, 1.0D, 0.5D), Vec3.createVectorHelper(0.5D, 0.5D, 0.0D),
        Vec3.createVectorHelper(0.5D, 0.5D, 1.0D), Vec3.createVectorHelper(0.0D, 0.5D, 0.5D),
        Vec3.createVectorHelper(1.0D, 0.5D, 0.5D) };

    public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {}

    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        RenderBlocks renderer) {
        return block instanceof BlockHeavyChain
            ? this.renderWorldBlock(world, x, y, z, (BlockHeavyChain) block, modelId, renderer)
            : false;
    }

    private boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, BlockHeavyChain block, int modelId,
        RenderBlocks renderer) {
        ...
        if (lowerDepth == 0.0D) {
            IAttachable attachable = GardenAPI.instance()
                .registries()
                .attachable()
                .getAttachable(lowerBlock, world.getBlockMetadata(x, y - 1, z));
