...
public class ContainerPotteryTable extends Container {
   ...
   public ContainerPotteryTable(InventoryPlayer inventory, TileEntityPotteryTable tileEntity) {
      ...
      this.outputSlot = this.addSlotToContainer(new SlotPottery(inventory.player, this.tableInventory, this.craftResult, 2, 110, 62));
      ...
      for(i = 0; i < 3; ++i) {
         for(int j = 0; j < 9; ++j) {
            this.playerSlots.add(this.addSlotToContainer(new Slot(inventory, j + i * 9 + 9, 10 + j * 18, 140 + i * 18)));
         }
      }
   ...
   public void onCraftMatrixChanged(IInventory inventory) {
      ...
      if (pattern != null && pattern.getItem() == Items.dye && target != null && target.getItem() == Item.getItemFromBlock(ModBlocks.largePot) && (target.getItemDamage() & 15) == 0) {
         this.craftResult.setInventorySlotContents(0, new ItemStack(ModBlocks.largePotColored, 1, target.getItemDamage() | pattern.getItemDamage()));
      } else {
         this.craftResult.setInventorySlotContents(0, PotteryManager.instance().getStampResult(pattern, target));
      }
   }
