...
public class BlockThinLog extends BlockContainer implements IChainSingleAttachable {
   ...
   private final Vec3[] attachPoints = new Vec3[]{Vec3.createVectorHelper(0.5D, (double)this.getMargin(), 0.5D), Vec3.createVectorHelper(0.5D, (double)(1.0F - this.getMargin()), 0.5D), Vec3.createVectorHelper(0.5D, 0.5D, (double)this.getMargin()), Vec3.createVectorHelper(0.5D, 0.5D, (double)(1.0F - this.getMargin())), Vec3.createVectorHelper((double)this.getMargin(), 0.5D, 0.5D), Vec3.createVectorHelper((double)(1.0F - this.getMargin()), 0.5D, 0.5D)};
   ...
   public int calcConnectionFlags(IBlockAccess world, int x, int y, int z) {
      ...
      boolean connectZPos = connectFlagsY == 0 && hardZPos || blockZPos == this && !hardConnectionZPos && (connectFlagsY != 3 || connectFlagsZPos != 3);
      boolean connectXNeg = connectFlagsY == 0 && hardXNeg || blockXNeg == this && !hardConnectionXNeg && (connectFlagsY != 3 || connectFlagsXNeg != 3);
      boolean connectXPos = connectFlagsY == 0 && hardXPos || blockXPos == this && !hardConnectionXPos && (connectFlagsY != 3 || connectFlagsXPos != 3);
   ...
   public void getSubBlocks(Item item, CreativeTabs creativeTabs, List blockList) {
      ...
      Iterator var7 = WoodRegistry.instance().registeredTypes().iterator();

      while(var7.hasNext()) {
         Entry entry = (Entry)var7.next();
         if (entry.getValue() != Blocks.log && entry.getValue() != Blocks.log2) {
            int id = TileEntityWoodProxy.composeMetadata((Block)entry.getValue(), ((UniqueMetaIdentifier)entry.getKey()).meta);
   ...
   public boolean addHitEffects(World worldObj, MovingObjectPosition target, EffectRenderer effectRenderer) {
      ...
      if (te != null && block != null) {
         ...
         float f = 0.1F;
         double xPos = (double)target.blockX + worldObj.rand.nextDouble() * (block.getBlockBoundsMaxX() - block.getBlockBoundsMinX() - (double)(f * 2.0F)) + (double)f + block.getBlockBoundsMinX();
         double yPos = (double)target.blockY + worldObj.rand.nextDouble() * (block.getBlockBoundsMaxY() - block.getBlockBoundsMinY() - (double)(f * 2.0F)) + (double)f + block.getBlockBoundsMinY();
         double zPos = (double)target.blockZ + worldObj.rand.nextDouble() * (block.getBlockBoundsMaxZ() - block.getBlockBoundsMinZ() - (double)(f * 2.0F)) + (double)f + block.getBlockBoundsMinZ();
         if (target.sideHit == 0) {
         ...
         fx.setParticleIcon(block.getIcon(worldObj.rand.nextInt(6), TileEntityWoodProxy.composeMetadata(protoBlock, protoMeta)));
            ...
            for(int ix = 0; ix < count; ++ix) {
               for(int iy = 0; iy < count; ++iy) {
                  for(int iz = 0; iz < count; ++iz) {
                     ...
                     fx.setParticleIcon(block.getIcon(world.rand.nextInt(6), TileEntityWoodProxy.composeMetadata(protoBlock, protoMeta)));
