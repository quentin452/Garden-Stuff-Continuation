...
public class WitcheryIntegration {
   ...
   public static void init() {
      if (Loader.isModLoaded("witchery")) {
         initWood();
         GardenCoreAPI.instance().registerBonemealHandler(new WitcheryIntegration.BonemealHandler());
         ...
         plantReg.registerPlantInfo("witchery", "leapinglily", new SimplePlantInfo(PlantType.AQUATIC_COVER, PlantSize.LARGE));
         plantReg.registerPlantInfo("witchery", "spanishmoss", new SimplePlantInfo(PlantType.HANGING_SIDE, PlantSize.LARGE));
         ...
         plantReg.registerPlantInfo("witchery", "voidbramble", new SimplePlantInfo(PlantType.GROUND, PlantSize.FULL));
         plantReg.registerPlantInfo("witchery", "seedsartichoke", 0, new SimplePlantInfo(PlantType.AQUATIC_SURFACE, PlantSize.LARGE));
         plantReg.registerPlantInfo("witchery", "seedswormwood", 0, new SimplePlantInfo(PlantType.GROUND, PlantSize.LARGE));
         plantReg.registerPlantInfo("witchery", "seedsmandrake", 0, new SimplePlantInfo(PlantType.GROUND, PlantSize.FULL));
         plantReg.registerPlantInfo("witchery", "seedswolfsbane", 0, new SimplePlantInfo(PlantType.GROUND, PlantSize.LARGE));
         plantReg.registerPlantInfo("witchery", "seedsbelladonna", 0, new SimplePlantInfo(PlantType.GROUND, PlantSize.FULL));
         plantReg.registerPlantInfo("witchery", "seedssnowbell", 0, new SimplePlantInfo(PlantType.GROUND, PlantSize.LARGE));
   ...
   private static class BonemealHandler implements IBonemealHandler {
      ...
      public boolean applyBonemeal(World world, int x, int y, int z, BlockGarden hostBlock, int slot) {
         ...
         if (plantItem == null) {
            ...
            ItemStack upgrade;
            if (plantItem.getItemDamage() < 4 && (plantItem.getItem() == GameRegistry.findItem("witchery", "seedsbelladonna") || plantItem.getItem() == GameRegistry.findItem("witchery", "seedsartichoke") || plantItem.getItem() == GameRegistry.findItem("witchery", "seedswormwood") || plantItem.getItem() == GameRegistry.findItem("witchery", "seedsmandrake") || plantItem.getItem() == GameRegistry.findItem("witchery", "seedssnowbell"))) {
            ...
            } else if (plantItem.getItemDamage() < 5 && plantItem.getItem() == GameRegistry.findItem("witchery", "garlic")) {
               upgrade = new ItemStack(plantItem.getItem(), 1, plantItem.getItemDamage() + 1);
               if (hostBlock.isPlantValidForSlot(world, x, y, z, slot, PlantItem.getForItem(world, upgrade))) {
                  te.setInventorySlotContents(slot, upgrade);
                  return true;
               }
            } else if (plantItem.getItemDamage() < 7 && plantItem.getItem() == GameRegistry.findItem("witchery", "seedswolfsbane")) {
               upgrade = new ItemStack(plantItem.getItem(), 1, plantItem.getItemDamage() + 1);
               if (hostBlock.isPlantValidForSlot(world, x, y, z, slot, PlantItem.getForItem(world, upgrade))) {
                  te.setInventorySlotContents(slot, upgrade);
                  return true;
