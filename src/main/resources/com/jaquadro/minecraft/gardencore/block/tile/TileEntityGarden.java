...
public class TileEntityGarden extends TileEntity implements IInventory {
   ...
   public boolean isSlotValid(int slot) {
      if (!this.isSharedSlot(slot)) {
         ...
         BlockGarden block = this.getGardenBlock();
         ISlotMapping[] var3 = block.getSlotShareProfile().getNeighborsForSlot(slot);
         int var4 = var3.length;

         for(int var5 = 0; var5 < var4; ++var5) {
            ISlotMapping mapping = var3[var5];
            if (!block.getConnectionProfile().isAttachedNeighbor(this.worldObj, this.xCoord, this.yCoord, this.zCoord, this.xCoord + mapping.getMappedX(), this.yCoord, this.zCoord + mapping.getMappedZ())) {
               return false;
            }
         }
   ...
   public boolean isAttachedNeighbor(int nx, int ny, int nz) {
      ...
      return garden == null ? false : garden.getConnectionProfile().isAttachedNeighbor(this.worldObj, this.xCoord, this.yCoord, this.zCoord, nx, ny, nz);
   }
   ...
   private ItemStack getStackLocal(int slot) {
      ...
      return this.containerPlants != null ? this.containerPlants[slot].getPlantSourceItem() : this.containerStacks[slot];
   }
   ...
   public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
      ...
      while(true) {
         ...
         this.getWorldObj().func_147479_m(this.xCoord, y, this.zCoord);
      }
   }
   ...
   public ItemStack getStackInSlot(int slot) {
      if (!this.isSharedSlot(slot)) {
         ...
         if (stack != null) {
            ...
            BlockGarden block = this.getGardenBlock();
            ISlotMapping[] var4 = block.getSlotShareProfile().getNeighborsForSlot(slot);
            int var5 = var4.length;

            for(int var6 = 0; var6 < var5; ++var6) {
               ISlotMapping mapping = var4[var6];
               TileEntity te = this.worldObj.getTileEntity(this.xCoord + mapping.getMappedX(), this.yCoord, this.zCoord + mapping.getMappedZ());
   ...
   public void setInventorySlotContents(int slot, ItemStack itemStack, boolean notify) {
      if (!this.isSharedSlot(slot)) {
         ...
         BlockGarden block = this.getGardenBlock();
         ISlotMapping[] var5 = block.getSlotShareProfile().getNeighborsForSlot(slot);
